...
class recuit_solver():
    def __init__(self,nbre_sol,time_per_sol):
        self.NBRE_RES = nbre_sol #Number of solutions that is going to be computed
        self.TIME_PER_SOL = time_per_sol #Number of minutes allowed to optimize a solution

    def solve(self):
        init_state = chromosome.tour()
        rec = recuit(init_state)
        auto_schedule = rec.auto(minutes=self.TIME_PER_SOL) #Get the parameters so that every solutions in computed in TIME_PER_SOL * minutes

        to_show = []
        #This loop compute the values with the annealar simulated algorithm
        for i in range(0,self.NBRE_RES):
            print(str(i)+"/"+str(self.NBRE_RES)+"\n")
        ...
        soltions_to_csv(pareto, "AnnealerSimulated2")
